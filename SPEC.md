...
- **Runner as a compose service.** `runner` runs as an ordinary container built from the repo-root `Dockerfile.runner-container` (release `ghcr.io/<owner>/noite-runner`). It owns the deploy pipeline, tenant celld fleets, the Caddyfile, telemetry aggregation and Git smart-HTTP. State lives in the `runner-data` volume plus the fleet bucket, and it talks straight to `rustfs:9000`.
