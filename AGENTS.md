...
- The control plane is **Oxide Worker UI on a celld node** (`apps/noite`, `preset: worker`) + **Rust runner as a plain compose service** (`apps/runner`, image from `docker/Dockerfile.runner`). The UI image bakes the built worker bundle (`apps/noite/dist`) and its entrypoint deploys it into `s3://<bucket>/control` on first boot (revision-gated, vars patched from the container env); the edge is stock upstream Caddy with a Caddyfile the runner writes. Every tenant app runs as its own celld fleet (prefix + keys); storage is a RustFS S3 bucket `s3://noite` with prefixes `git/`, `fleets/` and `control/`.
...
- `docker/` — **every Docker artifact lives here**: image definitions (`Dockerfile.runner` = the prod runner from `ghcr.io/<owner>/noite-runner`, `Dockerfile.ui` = the control UI, `Dockerfile.edge` = runner + Caddy in one container for Railway-style hosts that can't share a volume, `Dockerfile.runner-dev`, `Dockerfile.tools`), their per-Dockerfile context guards (`Dockerfile.<x>.dockerignore` — `<Dockerfile>.dockerignore` wins over a context-root `.dockerignore`, and patterns are relative to the repo-root context, so nested paths need `**/`), entrypoints (`noite-entrypoint.sh`, `edge-entrypoint.sh`, `install-sidecars.sh`, `control-dev.sh`, `runner-dev.sh`), Compose files (`compose.yaml` = the whole install, compose + Coolify via env; `compose.dev.yaml` dev overlay; `compose.byob.yaml` external-S3 overlay; `compose.e2e.yaml` raw-port test lane; `compose.standalone.yaml` = pull-only single-file mirror for cloud VMs and Compose stores, kept honest by `check-standalone.ts`), `doctor.sh`, `e2e-local.sh`, `backup.sh`, `restore.sh`, `check-versions.ts` (pin drift guard), `check-standalone.ts` (standalone-mirror drift guard). No `Dockerfile*`/`.dockerignore`/`compose*` may sit outside this directory. `standalone.ts`, `compose.standalone.yaml`, `Dockerfile.caddy`, `Caddyfile.static` and `railpack.json` are deleted. `Makefile` at the root.
